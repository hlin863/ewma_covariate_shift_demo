...
def resolve_dataset_2a_path(data_directory: str | Path, subject: int, session: str) -> Path:
    path = Path(data_directory) / dataset_2a_filename(subject, session)
    if not path.is_file():
        raise FileNotFoundError(
            "BCI Competition IV Dataset 2A file was not found: " f"{path}"
        )
    return path


def resolve_dataset_2b_path(data_directory: str | Path, subject: int, session: int) -> Path:
    path = Path(data_directory) / dataset_2b_filename(subject, session)
    if not path.is_file():
        raise FileNotFoundError(
            "BCI Competition IV Dataset 2B file was not found: " f"{path}"
        )
    return path
...
def _import_mne() -> Any:
    try:
        ...
        raise ImportError(
            "Loading GDF files requires MNE. Install it with: python -m pip install mne"
        ) from exc
    return mne
...
def _read_gdf(
    ...
    *,
    ...
    mne = _import_mne()
    raw = mne.io.read_raw_gdf(file_path, preload=preload, verbose="ERROR")
    if eeg_only:
    ...
    if signals.shape[0] != times.size:
        raise RuntimeError("Loaded EEG samples and time values must have equal length.")
...
def load_bci_competition_iv_2a_session(
    ...
    *,
    ...
    subject_value = _validate_subject(subject)
    session_value = _validate_session(session)
    file_path = resolve_dataset_2a_path(data_directory, subject_value, session_value)
    return _read_gdf(
        file_path,
...
def load_bci_competition_iv_2b_session(
    ...
    *,
    ...
    subject_value = _validate_subject(subject)
    session_value = _validate_dataset_2b_session(session)
    file_path = resolve_dataset_2b_path(data_directory, subject_value, session_value)
    suffix = _DATASET_2B_SESSION_SUFFIX[session_value]
    return _read_gdf(
        file_path,
        subject=subject_value,
...
def load_cse_feature_file(file_path: str | Path) -> tuple[np.ndarray, np.ndarray, np.ndarray | None]:
    path = Path(file_path)
    if not path.is_file():
    ...
    with np.load(path, allow_pickle=False) as contents:
        if "features" not in contents or "times" not in contents:
            raise ValueError("feature file must contain 'features' and 'times' arrays.")
        features = np.asarray(contents["features"], dtype=float)
        times = np.asarray(contents["times"])
        labels = np.asarray(contents["labels"]) if "labels" in contents else None
