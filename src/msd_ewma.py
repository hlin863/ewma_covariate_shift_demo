...
def calculate_multivariate_ewma_path(
    ...
    if initial_state.ndim != 1:
        raise ValueError(
            "initial_z must be one-dimensional."
        )

    if initial_state.shape != (observations.shape[1],):
        raise ValueError(
            "initial_z must have one value per feature."
        )

    if not np.isfinite(initial_state).all():
        raise ValueError(
            "initial_z must contain only finite values."
        )
    ...
    for index, observation in enumerate(observations):
        prediction_errors[index] = (
            observation - previous_z
        )
...
def fit_msd_ewma(
    ...
    observations = _validate_multivariate_values(
        training_values
    )

    initial_z = observations.mean(axis=0)

    ewma_states, prediction_errors = (
    ...
    inverse_error_covariance = np.linalg.pinv(
        error_covariance
    )
