...
class Dataset2BDiagnosticResult:
    ...
    def summary_row(self) -> dict[str, float | int | str | bool]:
        ...
        return {
            ...
            "control_limit_multiplier": float(
                warnings.iloc[0]["control_limit_multiplier"]
            ),
            "variance_update_mode": str(
                warnings.iloc[0]["variance_update_mode"]
            ),
    ...
    def warning_rows(self, testing: TrialFeatureResult) -> pd.DataFrame:
        ...
        return warnings.loc[warnings["stage_1_alarm"].astype(bool)].reset_index(
            drop=True
        )
...
def run_dataset_2b_diagnostic(
    ...
    *,
    ...
    """Run one Dataset 2B subject with explicit EWMA diagnostics."""
    ...
    published_lambda, published_csw, published_csv = PUBLISHED_2B_RESULTS[
        subject_id
    ]
