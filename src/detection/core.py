"""Dataset-agnostic covariate-shift estimation orchestration."""
...
from src.detection.stage2 import (
    HotellingConfig,
...
class CSEConfig:
    ...
    def __post_init__(self) -> None:
        ...
        if (
            ...
            raise ValueError("paper_two_sample requires equal validation window sizes.")
        ...
        if self.covariance_method not in {"empirical", "shrinkage"}:
            raise ValueError("covariance_method must be 'empirical' or 'shrinkage'.")
...
def _validate_cse_inputs(
    ...
    if training.shape[1] != testing.shape[1]:
        raise ValueError(
            "training_features and testing_features must have the same number of feature columns."
        )
    if times.ndim != 1:
        raise ValueError("testing_times must be one-dimensional.")
    if testing.shape[0] != times.size:
        raise ValueError(
            "testing_features and testing_times must contain the same number of observations."
        )
...
def _prepare_cse_features(
    ...
    *,
    ...
    if stage1_mode == "pc1_univariate":
        training_stage1 = extract_first_component(pca_result.training_transformed)
        testing_stage1 = extract_first_component(testing_transformed)
    else:
        training_stage1 = extract_retained_components(pca_result.training_transformed)
        testing_stage1 = extract_retained_components(testing_transformed)
    return pca_result, training_stage1, testing_transformed, testing_stage1
...
def _run_cse_warning_stage(
    ...
    if stage1_mode == "multivariate_pca":
        ...
        control_limit = float(
            chi2.ppf(1.0 - config.multivariate_alpha, df=n_features)
        )
        initial_z = (
        ...
        raise ValueError("stage1_mode must be 'pc1_univariate' or 'multivariate_pca'.")
...
def run_cse(
    ...
    pca_result, training_signal, testing_transformed, testing_signal = _prepare_cse_features(
    ...
    ewma_training_result, effective_lambda, warning_results = _run_cse_warning_stage(
