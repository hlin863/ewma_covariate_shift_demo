...
def _validate_multivariate_values(values: np.ndarray) -> np.ndarray:
...
def calculate_multivariate_ewma_path(
    ...
    initial_state = np.asarray(initial_z, dtype=float)
    ...
    ewma_states = np.empty_like(observations, dtype=float)
    prediction_errors = np.empty_like(observations, dtype=float)
...
def fit_msd_ewma(training_values: np.ndarray, lambda_value: float) -> MSDTrainingResult:
    ...
    error_covariance = np.atleast_2d(
        np.cov(prediction_errors, rowvar=False, ddof=0)
    )
...
def run_msd_ewma(
    ...
    *,
    ...
    inverse_covariance = np.asarray(inverse_error_covariance, dtype=float)
    records: list[dict[str, object]] = []
    for time_value, observation in zip(time_values, observations, strict=True):
