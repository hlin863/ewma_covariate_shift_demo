"""Filter-bank common spatial pattern features for BCI Competition IV 2B.

This module implements the feature path described in the CSE-UAEL paper:
ten overlapping 8--30 Hz bands, eighth-order zero-phase Butterworth filtering,
a separate binary CSP projection per band, and log-normalised variances from
the extreme CSP components.
"""
...
class CSPModel:
    """Projection matrix fitted for one frequency band."""
...
@dataclass(frozen=True)
class FBCSPModel:
    """Fitted filter-bank CSP transformer."""
...
def bandpass_trials(
    ...
    """Apply an eighth-order zero-phase Butterworth band-pass filter."""
...
def fit_binary_csp(filtered_trials: np.ndarray, labels: np.ndarray) -> np.ndarray:
    """Fit a binary CSP projection using class-mean normalised covariance."""
...
def _select_extreme_components(
    ...
    if 2 * components_per_side > n_components:
        raise ValueError(
            "twice components_per_side must not exceed the channel count."
        )
...
def log_normalised_variance(projected_trials: np.ndarray) -> np.ndarray:
    """Calculate Equation-11-style log-normalised CSP variances."""
...
def fit_fbcsp(
    ...
    *,
    ...
    """Fit one CSP model for every paper frequency band."""
...
def transform_fbcsp(trials: np.ndarray, model: FBCSPModel) -> np.ndarray:
    """Transform trials without refitting CSP or using evaluation labels."""
...
def fit_transform_fbcsp(
    ...
    *,
    ...
    """Fit on training trials and transform training and testing trials."""
