"""Run Dataset 2B CSE and compare observed values with published Table 1.

This script reports observed CSE outputs separately from the paper targets.
It never replaces computed values with published values. With the current
window-level band-power features, a mismatch is expected and is reported as
such; exact replication requires the paper's trial construction and feature
pipeline.
"""
...
def _parse_args():
    parser = ArgumentParser(
        description="Compare Dataset 2B CSE output with published Table 1."
    )
    parser.add_argument(
        "processed_directory",
        type=Path,
    ...
    parser.add_argument(
        "--output-file",
        type=Path,
        default=Path("outputs/metrics/bci_2b_table1_comparison.csv"),
    )
...
def _evaluate(training_file: Path, testing_file: Path, config: CSEConfig):
    ...
    return {
        ...
        "observed_csv": int(
            result.validation_results["confirmed_shift"].sum()
        ),
...
def main() -> None:
    ...
    display = [
        "subject", "paper_lambda", "observed_lambda",
        "paper_csw", "observed_csw", "paper_csv", "observed_csv",
        "replication_status",
    ]
    print(comparison[display].to_string(index=False))
    print(f"\nSaved comparison to: {args.output_file.resolve()}")
    print(
        "\nImportant: current observations are overlapping band-power windows, "
        "so this is a diagnostic comparison, not a claim of exact replication."
    )
